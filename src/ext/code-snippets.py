...
if ENV.GITHUB_TOKEN:
    GITHUB_HEADERS["Authorization"] = f"token {ENV.GITHUB_TOKEN}"
...
class CodeSnippets(BaseCog, hidden=True):
    ...
    async def _fetch_response(self, url: str, response_format: str, **kwargs: t.Any) -> t.Any:
        ...
        async with self.bot.client.get(url, raise_for_status=True, **kwargs) as response:
    ...
    def _find_ref(self, path: str, refs: tuple[dict[str, str]]) -> tuple[str, str]:
    ...
    def _snippet_to_codeblock(
        ...
        if end_line is None:
            start_line_num = end_line_num = int(start_line)
        else:
        ...
        if start_line_num > len(split_file_contents) or end_line_num < 1:
            return ""
        start_line_num = max(1, start_line_num)
        end_line_num = min(len(split_file_contents), end_line_num)

        # Gets the code lines, dedents them, and inserts zero-width spaces to prevent Markdown injection
        required = "\n".join(split_file_contents[start_line_num - 1 : end_line_num])
        ...
        for pattern, handler in self.pattern_handlers:
            for match in pattern.finditer(content):
                ...
                if normalized != unsanitized:
                    ...
                    if not match:
                        logger.info(
                            "Received code snippet url %s which " "attempted to circumvent url normalisation.",
                            unsanitized,
                        )
                        continue
                try:
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        destination = t.cast(discord.TextChannel, message.channel)
        ...
        if 0 < len(message_to_send) <= 2000:
            ...
            view.add_item(DynamicDeleteButton(user_id=message.author.id))
