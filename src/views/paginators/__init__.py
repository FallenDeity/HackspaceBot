...
__all__: tuple[str, ...] = ("BasePaginator",)
...
class BasePaginator(Generic[T], BaseView):
    ...
    async def send_page(self, ctx_or_inter: commands.Context[HackspaceBot] | discord.Interaction, page: T) -> None:
        ...
        # File
        file = discord.File(page) if not isinstance(page, discord.File) else discord.File(page.fp.name)  # type: ignore
        if self.message is None:
            return await self._send(ctx_or_inter, file=file, view=self)
        return await self._send(ctx_or_inter, file=file, view=self)

    async def start_paginator(
        self, ctx_or_inter: commands.Context[HackspaceBot] | discord.Interaction, *, starting_page: int = 0
    ) -> None:
