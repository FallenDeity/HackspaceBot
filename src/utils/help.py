...
class _InjectorCallback:
    ...
    async def invoke(self, *args: Any, **kwargs: Any) -> Any:
        ...
        return await self.callback.__func__(self.bind, *cleaned_args, **kwargs)


def _inject_callback(inject: "CustomHelpCommand") -> None:
    try:
    ...
    callback.__signature__ = Signature.from_callable(callback).replace(parameters=signature)  # type: ignore
...
def _construct_full_name(command: commands.Command[Any, ..., Any] | app_commands.Command[Any, ..., Any]) -> str:
    parent: Optional[app_commands.Group] = command.parent  # type: ignore
    entries: list[str] = []
...
class _HelpHybridCommandImpl(commands.HybridCommand[Cog, Any, Any]):
    ...
    async def prepare(self, ctx: Context[Any]) -> None:
        ...
        if not hasattr(on_error, "__help_command_not_overridden__"):
            if self.cog is not None:  # type: ignore
    ...
    async def _on_error_cog_implementation(self, _, ctx: Context[HackspaceBot], error: CommandError) -> None:
    ...
    def _eject_cog(self) -> None:
        if self.cog is None:  # type: ignore
            return
...
class Formatter:
    def __init__(self, help_command: "CustomHelpCommand") -> None:
    ...
    @staticmethod
    def __format_command_help(command: commands.Command[Any, ..., Any] | app_commands.Command[Any, ..., Any]) -> str:
        return command.description or "No help provided."

    @staticmethod
    def __format_command_aliases(command: commands.Command[Any, ..., Any] | app_commands.Command[Any, ..., Any]) -> str:
    ...
    @staticmethod
    def __format_command_enabled(command: commands.Command[Any, ..., Any] | app_commands.Command[Any, ..., Any]) -> str:
        if isinstance(command, app_commands.Command):
    ...
    def format_command(
        ...
        embed.set_thumbnail(url=self.ctx.bot.user.display_avatar if self.ctx.bot.user else "")
        return embed
    ...
    async def format_cog_or_group(
        ...
        cog_embed = (
            ...
            .set_thumbnail(url=self.ctx.bot.user.display_avatar if self.ctx.bot.user else "")
            .set_footer(
                text=f"Requested by {self.ctx.author}",
                icon_url=self.ctx.author.display_avatar,
            )
...
class CustomHelpCommand(commands.HelpCommand):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.command_attrs["with_app_command"] = with_app_command  # type: ignore
    ...
    def _remove_from_bot(self, bot: commands.bot.BotBase) -> None:
        ...
        app = cast(commands.hybrid.HybridAppCommand[Any, ..., Any], impl.app_command)
        for snowflake in cast(list[Snowflake], getattr(app, "_guild_ids", None) or []):
    ...
    async def send_bot_help(  # type: ignore
        self, mapping: Mapping[Optional[Cog], List[Command[Any, ..., Any] | app_commands.Command[Any, ..., Any]]], /
    ) -> None:
        home_embed = (
            discord.Embed(
                title="Home",
                description="Hackspace Bot Home Page - Custom Help Command",
                color=discord.Color.blue(),
            )
            .set_thumbnail(url=self.context.bot.user.display_avatar if self.context.bot.user else "")
            .set_footer(
        ...
        for _ in range(0, len(mapping), 5):
    ...
    async def command_callback(self, ctx: commands.Context[HackspaceBot], /, *, query: Optional[str] = None) -> None:  # type: ignore
        """
        ...
        if self.with_app_command:
            ...
            if cmd is None and guild_id is not None:
        ...
        for key in keys[1:]:
            try:
                cmds = getattr(cmd, "all_commands", None) or cmd._children  # type: ignore
                found = cmds.get(key) if cmds else None  # type: ignore
            except AttributeError:
    ...
    def get_app_command_mapping(
        ...
        def get_unbound_cmds(with_guild: discord.Guild | None = None):
            return [
                c
    ...
    async def help_command_autocomplete(
        ...
        help_command.context = cast(Context["HackspaceBot"], await inter.client.get_context(inter))
    ...
    async def filter_commands(  # type: ignore
        ...
        *,
        ...
        async def predicate(cmd: Command[Any, ..., Any] | app_commands.Command[Any, ..., Any]) -> bool:
            ctx = self.context
        ...
        ret: list[Command[Any, ..., Any] | app_commands.Command[Any, ..., Any]] = []
        for cmd in iterator:
